 ...
 *
 * vpackpath.c
 *	 Input/output and supporting routines for vpackpath
 *
 * vpackpath expression is a chain of path items.  First path item is $, $var,
 ...
 *					  1   5  7
 *
 * Binary encoding of vpackpath constitutes a sequence of 4-bytes aligned
 * variable-length path items connected by links.  Every item has a header
 * consisting of item type (enum VpackPathItemType) and offset of next item
 ...
 *
 * IDENTIFICATION
 *	src/backend/utils/adt/vpackpath.c
 *
 *-------------------------------------------------------------------------
 */
...
static Datum vpackPathFromCstring(char *in, int len);
static char *vpackPathToCstring(StringInfo out, VpackPath *in,
							   int estimated_len);
static int	flattenVpackPathParseItem(StringInfo buf, VpackPathParseItem *item,
									 int nestingLevel, bool insideArraySubscript);
static void alignStringInfoInt(StringInfo buf);
static int32 reserveSpaceForItemPointer(StringInfo buf);
static void printVpackPathItem(StringInfo buf, VpackPathItem *v, bool inKey,
							  bool printBracketes);
static int	operationPriority(VpackPathItemType op);


/**************************** INPUT/OUTPUT ********************************/

/*
 * vpackpath type input function
 */
PG_FUNCTION_INFO_V1(vpackpath_in);

Datum
vpackpath_in(PG_FUNCTION_ARGS)
{
	char	   *in = PG_GETARG_CSTRING(0);
	int			len = strlen(in);

	return vpackPathFromCstring(in, len);
}

/*
 * vpackpath type recv function
 *
 ...
 */
PG_FUNCTION_INFO_V1(vpackpath_recv);

Datum
vpackpath_recv(PG_FUNCTION_ARGS)
{
	...
	if (version == VPACKPATH_VERSION)
		str = pq_getmsgtext(buf, buf->len - buf->cursor, &nbytes);
	else
		elog(ERROR, "unsupported vpackpath version number: %d", version);

	return vpackPathFromCstring(str, nbytes);
}

/*
 * vpackpath type output function
 */
PG_FUNCTION_INFO_V1(vpackpath_out);

Datum
vpackpath_out(PG_FUNCTION_ARGS)
{
	VpackPath   *in = PG_GETARG_VPACKPATH_P(0);

	PG_RETURN_CSTRING(vpackPathToCstring(NULL, in, VARSIZE(in)));
}

/*
 * vpackpath type send function
 *
 * Just send vpackpath as a version number, then a string of text
 */
PG_FUNCTION_INFO_V1(vpackpath_send);

Datum
vpackpath_send(PG_FUNCTION_ARGS)
{
	VpackPath   *in = PG_GETARG_VPACKPATH_P(0);
	StringInfoData buf;
	StringInfoData jtext;
	int			version = VPACKPATH_VERSION;

	initStringInfo(&jtext);
	(void) vpackPathToCstring(&jtext, in, VARSIZE(in));
...
/*
 * Converts C-string to a vpackpath value.
 *
 * Uses vpackpath parser to turn string into an AST, then
 * flattenVpackPathParseItem() does second pass turning AST into binary
 * representation of vpackpath.
 */
static Datum
vpackPathFromCstring(char *in, int len)
{
	VpackPathParseResult *vpackpath = parsevpackpath(in, len);
	VpackPath   *res;
	StringInfoData buf;
	...
	appendStringInfoSpaces(&buf, VPACKPATH_HDRSZ);

	if (!vpackpath)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
				 errmsg("invalid input syntax for type %s: \"%s\"", "vpackpath",
						in)));

	flattenVpackPathParseItem(&buf, vpackpath->expr, 0, false);

	res = (VpackPath *) buf.data;
	SET_VARSIZE(res, buf.len);
	res->header = VPACKPATH_VERSION;
	if (vpackpath->lax)
		res->header |= VPACKPATH_LAX;

	PG_RETURN_VPACKPATH_P(res);
}

/*
 * Converts vpackpath value to a C-string.
 *
 * If 'out' argument is non-null, the resulting C-string is stored inside the
 * StringBuffer.  The resulting string is always returned.
 */
static char *
vpackPathToCstring(StringInfo out, VpackPath *in, int estimated_len)
{
	StringInfoData buf;
	VpackPathItem v;
	...
	if (!(in->header & VPACKPATH_LAX))
		appendBinaryStringInfo(out, "strict ", 7);

	jspInit(&v, in);
	printVpackPathItem(out, &v, false, true);

	return out->data;
}

/*
 * Recursive function converting given vpackpath parse item and all its
 * children into a binary representation.
 */
static int
flattenVpackPathParseItem(StringInfo buf, VpackPathParseItem *item,
						 int nestingLevel, bool insideArraySubscript)
{
	/* position from beginning of vpackpath data */
	int32		pos = buf->len - VPACKPATH_HDRSZ;
	int32		chld;
	int32		next;
	...
	switch (item->type)
	{
		...
		case jpiStartsWith:
			{
				...
				chld = !item->value.args.left ? pos :
					flattenVpackPathParseItem(buf, item->value.args.left,
											 nestingLevel + argNestingLevel,
											 insideArraySubscript);
				*(int32 *) (buf->data + left) = chld - pos;

				chld = !item->value.args.right ? pos :
					flattenVpackPathParseItem(buf, item->value.args.right,
											 nestingLevel + argNestingLevel,
											 insideArraySubscript);
		...
		case jpiLikeRegex:
			{
				...
				chld = flattenVpackPathParseItem(buf, item->value.like_regex.expr,
												nestingLevel,
												insideArraySubscript);
		...
		case jpiExists:
			{
				...
				chld = flattenVpackPathParseItem(buf, item->value.arg,
												nestingLevel + argNestingLevel,
												insideArraySubscript);
		...
		case jpiIndexArray:
			{
				...
				for (i = 0; i < nelems; i++)
				{
					...
					int32		topos;
					int32		frompos =
					flattenVpackPathParseItem(buf,
											 item->value.array.elems[i].from,
											 nestingLevel, true) - pos;

					if (item->value.array.elems[i].to)
						topos = flattenVpackPathParseItem(buf,
		...
		case jpiKeyValue:
			break;
		default:
			elog(ERROR, "unrecognized vpackpath item type: %d", item->type);
	}

	if (item->next)
	{
		chld = flattenVpackPathParseItem(buf, item->next, nestingLevel,
...
/*
 * Reserve space for int32 VpackPathItem pointer.  Now zero pointer is written,
...
/*
 * Prints text representation of given vpackpath item and all its children.
 */
static void
printVpackPathItem(StringInfo buf, VpackPathItem *v, bool inKey,
				  bool printBracketes)
{
	VpackPathItem elem;
	int			i;
	...
	switch (v->type)
	{
		...
		case jpiStartsWith:
			...
			printVpackPathItem(buf, &elem, false,
		...
		case jpiFilter:
			...
			jspGetArg(v, &elem);
			printVpackPathItem(buf, &elem, false, false);
			appendStringInfoChar(buf, ')');
			break;
		case jpiNot:
			appendBinaryStringInfo(buf, "!(", 2);
		...
		case jpiIndexArray:
			...
			for (i = 0; i < v->content.array.nelems; i++)
			{
				...
				printVpackPathItem(buf, &from, false, false);

				if (range)
				{
					appendBinaryStringInfo(buf, " to ", 4);
					printVpackPathItem(buf, &to, false, false);
				}
			}
			appendStringInfoChar(buf, ']');
			break;
		...
		case jpiKeyValue:
			...
			break;
		default:
			elog(ERROR, "unrecognized vpackpath item type: %d", v->type);
	}

	if (jspGetNext(v, &elem))
		printVpackPathItem(buf, &elem, true, true);
}

const char *
jspOperationName(VpackPathItemType type)
{
	switch (type)
	{
		case jpiAnd:
		...
		case jpiCeiling:
			return "ceiling";
		default:
			elog(ERROR, "unrecognized vpackpath item type: %d", type);
			return NULL;
	}
}

static int
operationPriority(VpackPathItemType op)
{
	switch (op)
	{
		case jpiOr:
...
/******************* Support functions for VpackPath *************************/
 ...
 */
void
jspInit(VpackPathItem *v, VpackPath *js)
{
	Assert((js->header & ~VPACKPATH_LAX) == VPACKPATH_VERSION);
 ...
 */
void
jspInitByBuffer(VpackPathItem *v, char *base, int32 pos)
{
	v->base = base + pos;
...
void
jspGetArg(VpackPathItem *v, VpackPathItem *a)
{
...
bool
jspGetNext(VpackPathItem *v, VpackPathItem *a)
{
	if (jspHasNext(v))
	{
...
void
jspGetLeftArg(VpackPathItem *v, VpackPathItem *a)
{
...
void
jspGetRightArg(VpackPathItem *v, VpackPathItem *a)
{
	Assert(v->type == jpiAnd ||
		   v->type == jpiOr ||
		   v->type == jpiEqual ||
...
char *
jspGetString(VpackPathItem *v, int32 *len)
{
...
bool
jspGetArraySubscript(VpackPathItem *v, VpackPathItem *from, VpackPathItem *to,
					 int i)
{
